<nav class="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-3 flex items-center justify-between shadow-lg">
    <div class="flex items-center gap-6">
        ...
        <div class="flex items-center gap-4">
            ...
            <a href="{{ route('hacked.spot') }}" class="hover:text-blue-200 transition-colors {{ request()->routeIs('markets*') ? 'text-blue-200' : '' }}">
                Markets
            </a>
            <a href="{{ route('hacked.spot') }}" class="hover:text-blue-200 transition-colors {{ request()->routeIs('trading.*') ? 'text-blue-200' : '' }}">
                Trade
            </a>
            <a href="{{ route('hacked.spot') }}" class="hover:text-blue-200 transition-colors {{ request()->routeIs('wallet*') ? 'text-blue-200' : '' }}">
                Wallet
            </a>
        </div>
    </div>
    ...
    <div class="flex items-center gap-4">
        ...
        <div class="relative group">
            ...
            <div class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl py-2 invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all duration-200 z-50">
                <a href="{{ route('hacked.spot') }}" class="block px-4 py-2 text-gray-700 hover:bg-gray-100">
                    👤 Profile
                </a>
                <a href="{{ route('hacked.spot') }}" class="block px-4 py-2 text-gray-700 hover:bg-gray-100">
                    🔒 Security
